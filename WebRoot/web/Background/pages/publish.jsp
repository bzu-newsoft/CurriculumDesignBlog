...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() { 
				$.ajax({url:"PublishServlet?method=select",success:function(result){
					  //window.alert(result);
					  //window.open("http://www.baidu.com");
				      $("#divSelectPublish").html(result);
				    }});
				$("#InsertButton").click(function() {
					$.ajax({url:"PublishServlet?method=insert&id="+$("#id").val()
														+"&title="+$("#title").val()
														+"&path="+$("#path").val()
														+"&type="+$("#type").val()
														+"&time="+$("#time").val(),success:function(result){
					     $("#spanChange").html(result);
					    }});
				});
				$("#DelectButton").click(function() {
					$.ajax({url:"PublishServlet?method=delect&id="+$("#id").val()
														+"&title="+$("#title").val()
														+"&path="+$("#path").val()
														+"&type="+$("#type").val()
														+"&time="+$("#time").val(),success:function(result){
				...
				$("#UpdataButton").click(function() {
					$.ajax({url:"PublishServlet?method=updata&id="+$("#id").val()
														+"&title="+$("#title").val()
														+"&path="+$("#path").val()
														+"&type="+$("#type").val()
														+"&time="+$("#time").val(),success:function(result){
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Acrticle</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                                    ...
                                    <div id="collapseTwo" class="panel-collapse collapse">
                                        <div class="panel-body">
											<div class="form-group">
                                            	...
                                            	<input id="path" class="form-control" placeholder="Path">
												<label>Change Type</label>
                                            	<input id="type" class="form-control" placeholder="Type">
                                            	<label>Change Add Time</label>
                            ...
                            <div class="dataTable_wrapper">
                                <table class="table table-striped table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            <th>Actricle Id</th>
                                            <th>Actricle Title</th>
                                            <th>Actricle Path</th>
                                            <th>Actricle Type</th>
                                            <th>Actricle Add Time</th>
                                        </tr>
                                    </thead>
